...
import { Calendar, Clock, Users, Award, ArrowLeft, Globe, Briefcase, Check, Ban } from "lucide-react";
      ...
      try {
        ...
        if (res.ok) {
          ...
          const enhancedData = data.map((event: Event) => ({
            ...event,
            organization: event.organization || ["TechCorp", "Microsoft", "Google", "Amazon", "IEEE"][Math.floor(Math.random() * 5)],
            logo: event.logo,
            prizes: event.prizes || "$" + (Math.floor(Math.random() * 10) + 1) * 1000,
            registrations_filled: event.registrations_filled || Math.floor(Math.random() * 80) + 10
          }));
        ...
        <div className="container mx-auto max-w-7xl px-4">
          <div className="text-center space-y-3 mb-8">
            ...
            <p className="text-lg text-blue-100 max-w-2xl mx-auto">
              Participate in prestigious hackathons, coding competitions and innovation challenges
            </p>
          </div>
          ...
          <Card className="max-w-3xl mx-auto shadow-lg border dark:border-gray-700 overflow-hidden">
            <div className="h-40 bg-gradient-to-r from-blue-600 to-indigo-700 dark:from-blue-800 dark:to-indigo-900 flex items-center justify-center relative overflow-hidden">
              ...
              <div className="text-white text-center p-4 z-10">
                <Avatar className="h-24 w-24 mx-auto bg-white p-1">
                  <AvatarImage src={selectedEvent.logo} alt={selectedEvent.organization} />
                  <AvatarFallback>{selectedEvent.organization?.slice(0, 2).toUpperCase()}</AvatarFallback>
                </Avatar>
              </div>
            </div>
            ...
            <CardHeader className="pt-6 pb-3 border-b dark:border-gray-700">
              <div className="flex items-center justify-between">
                <Badge variant={getStatusColor(selectedEvent.status) as any} className="flex items-center gap-1 px-3 py-1">
                  {getStatusIcon(selectedEvent.status)}
                  {selectedEvent.status}
                </Badge>
                <span className="text-sm text-gray-500 dark:text-gray-400">ID: #{selectedEvent.id}</span>
              </div>
              <CardTitle className="text-2xl font-bold mt-2">{selectedEvent.name}</CardTitle>
              <CardDescription className="text-sm mt-2 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <Briefcase className="h-4 w-4 text-gray-500" />
                  <span>Organized by <span className="font-semibold">{selectedEvent.organization}</span></span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {new Date(selectedEvent.start).toLocaleDateString(undefined, {
                      ...
                      month: 'long',
                      day: 'numeric'
                    })} to {new Date(selectedEvent.end).toLocaleDateString(undefined, {
                      weekday: 'long',
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
            ...
            <CardContent className="pt-6 space-y-6">
              <div className="grid md:grid-cols-3 gap-4 text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/30 p-4 rounded-lg">
                <div className="flex flex-col gap-2 items-center justify-center p-3 border-r dark:border-gray-700">
                  ...
                  <span className="text-xs uppercase font-medium text-gray-500">Team Size</span>
                  <span className="font-bold text-xl text-gray-900 dark:text-white">{selectedEvent.team_size}</span>
                </div>
                
                <div className="flex flex-col gap-2 items-center justify-center p-3 border-r dark:border-gray-700">
                  <Award className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                  <span className="text-xs uppercase font-medium text-gray-500">Prizes Worth</span>
                  <span className="font-bold text-xl text-gray-900 dark:text-white">{selectedEvent.prizes}</span>
                </div>
                
                <div className="flex flex-col gap-2 items-center justify-center p-3">
                  <Clock className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                  <span className="text-xs uppercase font-medium text-gray-500">Duration</span>
              ...
              <div className="space-y-6 pt-6 border-t dark:border-gray-700">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    />
                    {formErrors.teamName && <p className="text-xs text-red-500">{formErrors.teamName}</p>}
                  </div>
                  
                  <div className="space-y-2">
                    <Label htmlFor="team-members" className="text-sm font-medium">Team Members * <span className="text-xs text-gray-500">(comma-separated)</span></Label>
                    <Input
                      id="team-members"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="team-email" className="text-sm font-medium">Contact Email *</Label>
                    <Input
                      id="team-email"
                      type="email"
                    ...
                    />
                    {formErrors.teamEmail && <p className="text-xs text-red-500">{formErrors.teamEmail}</p>}
                  </div>
                  
                  <div className="space-y-2">
                    <Label htmlFor="additional-info" className="text-sm font-medium">Additional Information <span className="text-xs text-gray-500">(optional)</span></Label>
                    <Textarea
                      id="additional-info"
                  ...
                  <div className="pt-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        {formErrors.acceptedTerms && <p className="text-xs text-red-500 mt-1">{formErrors.acceptedTerms}</p>}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
              <Tabs defaultValue="all" className="w-full max-w-md">
                <TabsList className="grid w-full grid-cols-3">
                  ...
                  <TabsTrigger value="open" onClick={() => setFilter("open")}>Open Registration</TabsTrigger>
            ...
            {isLoading ? (
                  ...
                  >
                    <div className="h-36 bg-gradient-to-r from-blue-500 to-violet-500 flex items-center justify-center relative overflow-hidden">
                      <div className="absolute top-2 left-2">
                        <Badge variant={getStatusColor(ev.status) as any} className="font-medium flex items-center gap-1 px-2 py-1">
                      ...
                      <div className="text-white text-center p-4 z-10 flex flex-col items-center">
                        <Avatar className="h-16 w-16 mb-2 bg-white p-1">
                          ...
                          <AvatarFallback>{ev.organization?.slice(0, 2).toUpperCase()}</AvatarFallback>
                        </Avatar>
                    ...
                    <CardHeader className="pt-4 pb-2 flex-grow">
                      ...
                      <CardTitle className="text-lg font-bold line-clamp-1">{ev.name}</CardTitle>
                      ...
                      <div className="mt-3 space-y-3">
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between text-xs">
                            ...
                            <span className="font-medium">{ev.registrations_filled}% filled</span>
                          </div>
                          <Progress value={ev.registrations_filled} className="h-1" />
                        </div>
                      </div>
                    </CardHeader>
                    ...
                    <CardFooter className="pt-2 border-t dark:border-gray-700 mt-auto">
                      ...
                      >
                        {ev.status.toLowerCase().includes("open") ? "Register Now" : "Closed"}
                      </Button>
                    </CardFooter>
                  </Card>
                ))}
              ...
              <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow">
                ...
                <h3 className="text-lg font-medium mb-2 text-gray-700 dark:text-gray-300">No events found</h3>
                <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6">
                  We couldn't find any events matching your criteria. Try adjusting your filters or check back later for new opportunities.
                </p>
                <Button 
                  variant="outline" 
                  onClick={() => {
